...
class TweetANew {
	...
	public static function TweetANewEditMade( &$article, &$user, $text, $summary, $minoredit, $watchthis,
		...
		if ( $wgRequest->getCheck( 'wpTweetANewEdit' ) || $wgTweetANewTweet['Edit'] ) {
			...
			# 2012-08-10 DERF show diff URL
			if ($revision) {
				$finalurl = self::makeFinalUrl(
					'https://wiki.chaosdorf.de/index.php?title=' . $article->getTitle()->escapeLocalURL() . '&diff=prev&oldid=' . $revision->getId()
				);
			} else {
				$finalurl = self::makeFinalUrl($article->getTitle()->getFullURL());
			}
	...
	public static function TweetANewUploadMade( &$image ) {
		global $wgTweetANewTweet, $wgTweetANewText, $wgRequest;
		if ( $wgTweetANewTweet['Upload'] ) {
			$finalurl = self::makeFinalUrl($image->getLocalFile()->getTitle()->getFullURL());
			$author = $image->getLocalFile()->user_text;
			$tweet_text = "$author uploaded $finalurl ("
				. $image->getLocalFile()->description . ")";
			self::makeSendTweet($tweet_text, $finalurl);
		}
		return true;
	}
